...
# from sources.nets_old import cross_network
from sources.build_longitudinal_nets import build_network
...
def da_generator_Longitudinal(x_train, y_train, numsample, n_dim, batch_size=256):
    """
...
@threadsafe_generator
def val_generator_Longitudinal(x_train_v, y_train_v, numsample, n_dim, batch_size=256):
    """
    Keras generator used for training with data augmentation. This generator
    calls the data augmentation function yielding training samples
    """
...
def build_and_compile_models_tensor_2(settings):
    ...
    with strategy.scope():
        model = build_network(settings, first)
        # try:
        ...
        for i in range(0, int(this_size)):
            all_loss.append(concatenated_loss)
        ...
        model2 = build_network(settings, second)
        # try:
        ...
        if settings['full_train'] is False:
            ...
            print("> CNN: Loading learnedmodel weights from the", settings['learnedmodel_model'], "configuration")
...
def define_training_layers(model, settings, num_layers=1, number_of_samples=None):
...
def train_model(model, x_train, y_train, val_x_train, val_y_train, settings, initial_epoch=0):
    ...
    for label in range(0, this_size):
        ...
        val_y_train[label] = keras.utils.to_categorical(val_y_train[label] == 1,
                                                   len(np.unique(val_y_train[label] == 1)))
    ...
    for label in range(0, this_size):
        ...
        train_val_v[label] = int(val_x_train[label].shape[0])
    ...
    for i in range(0, this_size):
        ...
        x_train_[i] = x_train[i][:train_val]
        # print("x_train_[", i, "]:", x_train_[i].shape[0])
        # print("y_train[", i, "]:", y_train[i].shape[0])
        y_train_[i] = y_train[i][:train_val]
        # print("y_train_[", i, "]:", y_train_[i].shape[0])
        # print("x_val", i, "]:", X_val[i].shape[0])
        x_val_[i] = val_x_train[i][:train_val_v]
        # print("x_val_", i, "]:", x_val_[i].shape[0])
        # print("y_val[", i, "]:", Y_val[i].shape[0])
        y_val_[i] = val_y_train[i][:train_val_v]
        # print("y_val_[", i, "]:", y_val_[i].shape[0])

    numsamplex = calcul_num_sample_multi_class(x_train_, this_size)
    # numsamplex = numsamplex - this_size * batch_size
    numsamplexv = calcul_num_sample_multi_class(x_val_, this_size)
    # numsamplexv = numsamplexv - this_size * batch_size
    print("Number of sample for Longitudinal training:", numsamplex)
    print("Number of sample for Longitudinal cross validation:", numsamplexv)
    # da_generator_multi_class(x_train, y_train, numsample, n_dim, batch_size=256)
    history = model['model'].fit_generator(da_generator_Longitudinal(
        x_train_, y_train_, numsamplex, this_size,
        batch_size=batch_size),
        validation_data=val_generator_Longitudinal(x_val_, y_val_, numsamplexv, this_size, batch_size=batch_size),
