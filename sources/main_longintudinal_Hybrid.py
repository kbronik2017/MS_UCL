...
from sources.build_model_Longitudinal_Hybrid import define_training_layers, train_model, fit_thismodel
...
def train_sec_model(secmodel, firstmodel, train_x_data, train_y_data, val_x_data, val_y_data, settings, thispath):
    ...
    if settings['full_train'] is False:
        ...
        for it in range(0, max_epochs, 10):
            ...
            settings['balanced_training'] = True
            Val_X, Val_Y, Val_sel_voxels = dataset_training(val_x_data, val_y_data,
                                                  settings,
                                                  model=firstmodel,
...
def dataset_training(train_x_data,
    ...
    if model is None:
        print('model is None')
        for i in range(0, this_size):
            flair_scans[i] = [train_x_data[s][i+1] for s in scans]
            selected_voxels_tmp[i] = select_training_voxels(flair_scans[i],
                                                        settings['min_th'])
    elif selected_voxels is None:
        print('select voxels from previous model')
        t_x_data = {}
        for n, i in zip(range(1, this_size + 1), range(0, this_size)):
            # y_data[i] = [train_y_data[s][n] for s in scans]
            t_x_data[i] = {s: train_x_data[s][n] for s in scans}
        for i in range(0, this_size):
            selected_voxels_tmp[i] = voxels_from_learned_model(model,
                                                                   t_x_data[i],
                                                                   settings,  index=i)

    else:
        print('selection of  voxels done unsuccessfully!!!')
        pass
    ...
    if settings['balanced_training'] is True:
        for i in range(0, this_size):
            x_data = [train_x_data[s][i + 1] for s in scans]
            y_data = [train_y_data[s][i + 1] for s in scans]
            print('Input X:', x_data)
            print('Input Y:', y_data)
            x_patches, y_patches = dataset_train_patches(x_data,
                                                      y_data,
                                                      selected_voxels_tmp[i],
                                                      settings['patch_size'],
                                                      settings['balanced_training'],
                                                      settings['ratio_negative_positive1'])
            print('Input X loaded array dim:', x_patches.shape)
            print('Input Y loaded array dim:', y_patches.shape)
            data_x.append(x_patches)
            data_y.append(y_patches)
    ...
    data_xx = np.concatenate(data_x, axis=0)
    Y = np.concatenate(data_y, axis=0)
    print('Concatenate Input X dim:', data_xx.shape)
    print('Concatenate Input Y dim:', Y.shape)
    X = np.expand_dims(data_xx, axis=1)
    # X = np.stack(data_xx, axis=1)
    # Y = np.stack(data_yy, axis=1)
...
def dataset_train_patches(x_data,
    ...
    print('Load train patches of X:', x_data)
    print('Load train patches of Y:', y_data)
    # print('selected_voxels:', len(selected_voxels))
    # print('selected_voxels:', selected_voxels)
...
def prediction(model,
    ...
    scans = list(test_x_data.keys())
    flair_scans = [test_x_data[s] for s in scans]
    print('testing >>>>', test_x_data)
    print('flair_scans:', flair_scans[0])
    flair_image = load_nii(flair_scans[0])
    ...
    if settings['debug'] is True:
        print("> DEBUG: testing current_batch[", index + 1, "]:", batch.shape, end=' ')
    ...
    #                               settings['batch_size'])
    y_pred_all = model['model'].predict(batch, settings['batch_size'])
